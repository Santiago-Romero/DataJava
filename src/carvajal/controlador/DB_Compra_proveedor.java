 ...
 */
public class DB_Compra_proveedor {

    FileReader fr;
    String nomFile;
    boolean error = false;

    public DB_Compra_proveedor(String nomFile) {
    ...
    public void guardar() {
        if (!error) {
            ...
            sw = con.actualizar("DELETE FROM compra_proveedor;");
            if (!sw) {
                try {
                    while ((registro = br.readLine()) != null) {
                        String tokens[] = registro.split(";");
                        sw = con.actualizar("INSERT INTO compra_proveedor values('"
                                + tokens[0] + "', '"
                                + tokens[1] + "', '"
                                + tokens[2] + "', '"
                                + tokens[3] + "', '"
