 ...
 */
public class DB_Ventas_empaques {

    FileReader fr;
    String nomFile;
    boolean error = false;

    public DB_Ventas_empaques(String nomFile) {
    ...
    public void guardar() {
        if (!error) {
            ...
            sw = con.actualizar("DELETE FROM ventas_empaques;");
            if (!sw) {
                try {
                    while ((registro = br.readLine()) != null) {
                        String tokens[] = registro.split(";");
                        sw = con.actualizar("INSERT INTO ventas_empaques values('"
                                + tokens[0] + "', '"
                                + tokens[1] + "', '"
                                + tokens[2] + "', '"
                                + tokens[3] + "', '"
