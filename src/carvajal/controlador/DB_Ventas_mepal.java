 ...
 */
public class DB_Ventas_mepal {

    FileReader fr;
    String nomFile;
    boolean error = false;

    public DB_Ventas_mepal(String nomFile) {
    ...
    public void guardar() {
        if (!error) {
            ...
            sw = con.actualizar("DELETE FROM ventas_mepal;");
            if (!sw) {
                try {
                    while ((registro = br.readLine()) != null) {
                        String tokens[] = registro.split(";");
                        sw = con.actualizar("INSERT INTO ventas_mepal values('"
                                + tokens[0] + "', '"
                                + tokens[1] + "', '"
                                + tokens[2] + "', '"
                                + tokens[3] + "', '"
