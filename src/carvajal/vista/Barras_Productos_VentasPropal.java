...
public class Barras_Productos_VentasPropal extends ApplicationFrame {

    public ChartPanel panel;
    public JFreeChart chart;
    int producto1, producto2, producto3, producto4, producto5;
    int dinero;
    public Barras_Productos_VentasPropal(String title,int dinero) throws SQLException {
        super(title);
        this.dinero=dinero;
    ...
    private CategoryDataset crearDataset() throws SQLException {
        ...
        if (!error) {
            ResultSet rs = con.consulta("SELECT * FROM ventas_propal");
            while(rs.next()){
            if(dinero==0){
                ...
                if(dinero==1){
                    if(rs.getInt(3)==(1)) producto1+=(rs.getInt(4)*10);
                    if(rs.getInt(3)==(2)) producto2+=(rs.getInt(4)*15);
                    if(rs.getInt(3)==(3)) producto3+=(rs.getInt(4)*13);
                    if(rs.getInt(3)==(4)) producto4+=(rs.getInt(4)*20);
                    if(rs.getInt(3)==(5)) producto5+=(rs.getInt(4)*20);
                }
                if(dinero==2){
                    if(rs.getInt(3)==(1)) producto1+=(rs.getInt(4)*1);
                    if(rs.getInt(3)==(2)) producto2+=(rs.getInt(4)*2);
                    if(rs.getInt(3)==(3)) producto3+=(rs.getInt(4)*1);
        ...
        con.cerrarConsulta();
        con.desconectar();
        dataset.setValue(producto1, "Ventas", "Papel fotografia");
        dataset.setValue(producto2, "Ventas", "Papel esmaltado");
        dataset.setValue(producto3, "Ventas", "Papel no esmaltado");
        dataset.setValue(producto4, "Ventas", "Propalcopia");
        dataset.setValue(producto5, "Ventas", "Propalpac");
