...
public class Barras_Sucursal_VentasEmpaques extends ApplicationFrame {

    public ChartPanel panel;
    public JFreeChart chart;
    int sucursal1, sucursal2, sucursal3, sucursal4, sucursal5;

    public Barras_Sucursal_VentasEmpaques(String title) throws SQLException {
        super(title);
    ...
    private CategoryDataset crearDataset() throws SQLException {
        ...
        if (!error) {
            ResultSet rs = con.consulta("SELECT sucursal FROM ventas_empaques");
            while(rs.next()){
            if(rs.getString(1).equals("Bogota")) sucursal1++;
            if(rs.getString(1).equals("Tocancipa")) sucursal2++;
            if(rs.getString(1).equals("Cali")) sucursal3++;
            if(rs.getString(1).equals("Ginebra")) sucursal4++;
            if(rs.getString(1).equals("Medellin")) sucursal5++;
            }
        }
        con.cerrarConsulta();
        con.desconectar();
        dataset.setValue(sucursal1, "Ventas", "Bogota");
        dataset.setValue(sucursal2, "Ventas", "Tocancipa");
        dataset.setValue(sucursal3, "Ventas", "Cali");
        dataset.setValue(sucursal4, "Ventas", "Ginebra");
