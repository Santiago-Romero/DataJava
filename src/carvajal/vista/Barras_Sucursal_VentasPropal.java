...
public class Barras_Sucursal_VentasPropal extends ApplicationFrame {

    public ChartPanel panel;
    public JFreeChart chart;
    int sucursal1;

    public Barras_Sucursal_VentasPropal(String title) throws SQLException {
        super(title);
    ...
    private CategoryDataset crearDataset() throws SQLException {
        ...
        if (!error) {
            ResultSet rs = con.consulta("SELECT sucursal FROM ventas_propal");
            while(rs.next()){
            if(rs.getString(1).equals("Yumbo")) sucursal1++;
            }
        }
        con.cerrarConsulta();
        con.desconectar();
        dataset.setValue(sucursal1, "Ventas", "Yumbo");
