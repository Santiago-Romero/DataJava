...
public class Barras_Sucursal_VentasSolCom extends ApplicationFrame {

    public ChartPanel panel;
    public JFreeChart chart;
    int sucursal1, sucursal2, sucursal3, sucursal4, sucursal5, sucursal6, sucursal7;

    public Barras_Sucursal_VentasSolCom(String title) throws SQLException {
        super(title);
    ...
    private CategoryDataset crearDataset() throws SQLException {
        ...
        if (!error) {
            ResultSet rs = con.consulta("SELECT sucursal FROM ventas_sol_com");
            while(rs.next()){
            if(rs.getString(1).equals("Bogota")) sucursal1++;
            if(rs.getString(1).equals("Medellin")) sucursal2++;
            if(rs.getString(1).equals("Bucaramanga")) sucursal3++;
            if(rs.getString(1).equals("Manizales")) sucursal4++;
            if(rs.getString(1).equals("Cali")) sucursal5++;
            if(rs.getString(1).equals("Barranquilla")) sucursal6++;
            if(rs.getString(1).equals("Pereira")) sucursal7++;
            }
        }
        con.cerrarConsulta();
        con.desconectar();
        ...
        dataset.setValue(sucursal3, "Ventas", "Bucaramanga");
        dataset.setValue(sucursal4, "Ventas", "Manizales");
        dataset.setValue(sucursal5, "Ventas", "Cali");
        dataset.setValue(sucursal6, "Ventas", "Barranquilla");
        dataset.setValue(sucursal7, "Ventas", "Pereira");
